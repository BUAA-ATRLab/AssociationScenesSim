...
function [XReentryS,YReentryS,ZReentryS,VXReentryS,VYReentryS,VZReentryS,thetaXReentryS,thetaYReentryS,thetaZReentryS,num3]=...
    ...
    ManeuverStartTime=settings.ManeuverStartTime/delta_t;
    ManeuvergY = settings.ManeuvergY;
    ManeuvergZ = settings.ManeuvergZ;
    ManeuverVzmax = settings.ManeuverVzmax;
    ...
    for i = 2:num_point3  % Reentry section 段航迹
        if i < ManeuverStartTime
            gY = 0;
        else
            gY = ManeuvergY;
        end
		...
		kk12=gY/V3_t(i-1)+(V3_t(i-1)/(h3_t(i-1)+R_R)-g0/V3_t(i-1))*cos(theta3_t(i-1));
		...
		kk22=gY/(V3_t(i-1)+0.5*delta_t*kk11)+((V3_t(i-1)+0.5*delta_t*kk11)/((h3_t(i-1)+0.5*delta_t*kk13)+R_R)-g0/(V3_t(i-1)+0.5*delta_t*kk11))*cos((theta3_t(i-1)+0.5*delta_t*kk12));
		...
		kk32=gY/(V3_t(i-1)+0.5*delta_t*kk21)+((V3_t(i-1)+0.5*delta_t*kk21)/((h3_t(i-1)+0.5*delta_t*kk23)+R_R)-g0/(V3_t(i-1)+0.5*delta_t*kk21))*cos((theta3_t(i-1)+0.5*delta_t*kk22));
		...
		kk42=gY/(V3_t(i-1)+delta_t*kk31)+((V3_t(i-1)+delta_t*kk31)/((h3_t(i-1)+delta_t*kk33)+R_R)-g0/(V3_t(i-1)+delta_t*kk31))*cos((theta3_t(i-1)+delta_t*kk32));
    ...
    for i = 1:num_point3
        ...
        if i < ManeuverStartTime
            VZReentryG(i) = 0;
        elseif i > ManeuverVzmax/ManeuvergZ/delta_t + ManeuverStartTime
            VZReentryG(i) = VZReentryG(i-1);
        else
            VZReentryG(i) = ManeuvergZ*(i - ManeuverStartTime)*delta_t;
        end
    end
    ...
    for i = 1:num_point3
        if i > num3
            ...
            VCoordS = G2S_2(launchBLA,[VXReentryG(i),VYReentryG(i),VZReentryG(i)]);
