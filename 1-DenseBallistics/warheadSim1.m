...
%numGroup：团编号
%弹道导弹弹道学
function [] = warheadSim1(launchBLA,launchLandLatiLong,numGroup,settings,environment)

    numTrajectory=settings.iMisNum;
    numSecondLaunchTime=settings.numSecondLaunchTime;
    numThirdLaunchTime=settings.numThirdLaunchTime;
    ...
    str3='.\mid\Missiles_Track\yer_help';
	ch17=num2str(numTrajectory);
	str4=[ch17,'.txt'];
    str3A = [str3,str4];
    
	fp_help5 = fopen(str3A, 'w');
    if (fp_help5==-1)
		error(['cannot open ',str3A, '\n']);
    end
    
	fprintf(fp_help5,'%15.5f %15.5f\n',wz,e_e);
	fclose(fp_help5);
    ...
    if numSecondLaunchTime~=0
		%生成一个文件，内容是第二个warhead起始时刻，位置，速度，trajectory倾角（launch坐标系下）（二三warhead要分开）
		str5='.\mid\Missiles_Track\warhead2_start';
        ch5=num2str(numTrajectory);
		str6=[ch5,'.txt'];
        str5A = [str5,str6];

        fp_help32 = fopen(str5A, 'w');
        if (fp_help32==-1)
            error(['cannot open ',str5A, '\n']);
        end
    
		fprintf(fp_help32,'%10.4f',t_2(numSecondLaunchTime));
		fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',XFreeG(numSecondLaunchTime));
        fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',YFreeG(numSecondLaunchTime));
        fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',0.0);
		fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',VxFree(numSecondLaunchTime));
        fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',VyFree(numSecondLaunchTime));
        fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',0.0);
		fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',thetaXFree(numSecondLaunchTime));
        fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',thetaYFree(numSecondLaunchTime));
        fprintf(fp_help32,'%5s','');fprintf(fp_help32,'%15.5f',0.0);
		fprintf(fp_help32,'\n');
	...
	if numThirdLaunchTime~=0
		%生成一个文件，内容是第三个warhead起始时刻，位置，速度，trajectory倾角（launch坐标系下）（二三warhead要分开）
		str7='.\mid\Missiles_Track\warhead3_start';
        ch6=num2str(numTrajectory);
		str8=[ch6,'.txt'];
        str7A = [str7,str8];
        
        fp_help33 = fopen(str7A, 'w');
        if (fp_help33==-1)
            error(['cannot open ',str7A, '\n']);
        end
    		
		fprintf(fp_help33,'%10.4f',t_2(numThirdLaunchTime));
		fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',XFreeG(numThirdLaunchTime));
        fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',YFreeG(numThirdLaunchTime));
        fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',0.0);
		fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',VxFree(numThirdLaunchTime));
        fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',VyFree(numThirdLaunchTime));
        fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',0.0);
		fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',thetaXFree(numThirdLaunchTime));
        fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',thetaYFree(numThirdLaunchTime));
        fprintf(fp_help33,'%5s','');fprintf(fp_help33,'%15.5f',0.0);
		fprintf(fp_help33,'\n');
    ...
    if rangeBMkm<=2000
		num_point3=round(num_point3*3);
	else
		num_point3=round(num_point3*1.5);%%%%%%/用于改正landing point不在地面的情况%%%%%%%%%%
    end
    ...
    str1='.\mid\Missiles_Track\warhead';
    ...
    % 存储第2个文件
    % 存储每一段的数据点数
	str9='.\mid\Missiles_Track\PointNumber';
	ch7=num2str(numTrajectory);
	str10=[ch7,'.txt'];
	str9A = [str9,str10];

    fp_help = fopen(str9A, 'w');%总点数
    ...
    % 第二个弹头的发射时的真近点角和轨道偏角
	if numSecondLaunchTime~=0
		str11='.\mid\Missiles_Track\param2_mid';
        ch8=num2str(numTrajectory);
		str12=[ch8,'.txt'];
        str11A = [str11,str12];
        fp_help22 = fopen(str11A, 'w');%warhead二和warhead三要分开（各有用中间参数）
       
        if (fp_help22==-1)
            error(['cannot open ',str11A, '\n']);
        end
		
        fSecondLaunch=fFree(numSecondLaunchTime);
        thetaSecondLaunch=thetaFree(numSecondLaunchTime);
    ...
    % 第三个弹头的发射时的真近点角和轨道偏角
	if numThirdLaunchTime~=0
		str11='.\mid\Missiles_Track\param3_mid';
		ch8=num2str(numTrajectory);
		str12=[ch8,'.txt'];
		str11A = [str11,str12];
		fp_help23 = fopen(str11A, 'w');%warhead二和warhead三要分开（各有用中间参数）        
       
        if (fp_help23==-1)
            error(['cannot open ',str11A, '\n']);
        end
		fThirdLaunch=fFree(numThirdLaunchTime);
        thetaThirdLaunch=thetaFree(numThirdLaunchTime);
    ...
    % 存储第5个文件
    % 时间序列
	str13='.\mid\Missiles_Track\time';
    ch9=num2str(numTrajectory);
	str14=[ch9,'.txt'];
    str13A = [str13,str14];

    fp_help4 = fopen(str13A, 'w');
    
    if (fp_help4==-1)
        error(['cannot open ',str13A, '\n']);
    end

	for i=1:numTotal
		fprintf(fp_help4,'%10.5f\n',tTotal(i));
    end
